...
function Category() {
	...
	const {
		...
		isLoading,
		error,
	} = useQuery(['paginatedData', currentPage, categoryName], async () => {
				...
				<div className=''>
					<header className='flex-col items-start '>
						...
						<div className='h-[300px] bg-slate-50 p-3 rounded-xl w-full md:w-max shadow-lg'>
							<h1 className='text-[25px] md:text-[35px] my-3 font-bold text-blue-700 '>
								Rent {params.categoryName} equipment in Gandhinagar
							</h1>
							<p className='tex-[16px] font-light'>
								Hire photography equipments from fellow photographer in
								Gandhinagar, over 50% cheaper than traditional rental houses{' '}
							</p>
						</div>
					</header>

					{isLoading ? (
						<div className='grid gap-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 md:p-3 '>
							{[...Array(10).keys()].map((i) => (
								<SkeletonPost />
							))}
						</div>
						...
						<>
							<main className='my-12'>
								<div className='z-0 grid gap-2  sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4'>
									{listings.map(
										(listing) =>
