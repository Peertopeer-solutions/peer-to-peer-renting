...
import { Content } from "../components/Design/Typography";
...
const OrderConfirmation = () => {
  ...
  const stripePublishableKey =
    "pk_test_51IdIGxSGDQdSXagEB1lpJvTj9VtOCRbNXswtm30dAKh1Lj0uwVw1tQHYzvt1f6GhWGgqPRKdO35ZZG80PctbCHUS00d7BLFyA3";
  ...
  useEffect(() => {
    ...
    const createStripeCheckout = () => {
      const lineItems = [
        {
          ...
          price_data: {
            currency: "inr",
            unit_amount:
              ((product?.regularPrice * request?.rentalPeriod) / 2 +
                product?.regularPrice * request?.rentalPeriod) *
              100, // 10000 = 100 USD
            product_data: {
              name: product?.title,
            },
      ...
      fetch(
        "https://us-central1-awesome-renting.cloudfunctions.net/createStripeCheckout",
        {
          method: "POST",
          headers: {
  ...
  const date = new Date(order.data.timestamp?.seconds * 1000).toLocaleString(
    "en-US",
    options
  );
  const startDate = new Date(request.startDate?.seconds * 1000).toLocaleString(
    "en-US",
    options
  );
  const endDate = new Date(request.endDate?.seconds * 1000).toLocaleString(
    "en-US",
    options
  );
  ...
  return (
    <div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
      <div className="flex justify-start item-start space-y-2 flex-col ">
        <h1 className="text-3xl lg:text-4xl font-semibold leading-7 lg:leading-9  text-gray-800 uppercase">
          Order #{uniqueOrderId}
        </h1>
      ...
      <div className="mt-3">
        <h1 className="text-red-500">
          {" "}
          Please confirm order by clicking on this link
        </h1>
            ...
            <div className="mt-4 md:mt-6 flex  flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full ">
              ...
              <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full  pb-8 space-y-4 md:space-y-0">
                <div className="w-full flex flex-col justify-start items-start space-y-8">
                  ...
                  <div className="flex justify-start items-start flex-col space-y-2">
                    <p className="text-sm leading-none text-gray-800">
                      <span className="text-gray-300">Rental start: </span>{" "}
                      {startDate}
                    </p>
                    <p className="text-sm leading-none text-gray-800">
                      <span className="text-gray-300">Rental End: </span>{" "}
                      {endDate}
                    </p>
                  </div>
                </div>
                <div className="flex justify-between space-x-8 items-start w-full">
                  <Content>
                    Payment <span className="text-yellow-500">Pending</span>
                  </Content>
                  <Content className="xl:text-lg">
                    Rs {product.regularPrice * request.rentalPeriod}{" "}
